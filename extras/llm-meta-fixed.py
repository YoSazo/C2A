#!/usr/bin/env python3
"""
LLM Meta - C2A Strategic Oracle (FIXED VERSION)
...
- Fallback to text extraction when OCR isn't needed
"""
...
try:
    ...
    OCR_SUPPORT = False
    print("⚠️  OCR support not available. Install: pip install pytesseract pillow")
...
class LLMMetaEngine:
    ...
    def __init__(self):
        ...
        print(f"  OCR Support: {'✓' if OCR_SUPPORT else '✗ (install pytesseract + pillow)'}")
    ...
    def _read_file(self, file_path):
        """Read text from file (supports .txt and .pdf with OCR fallback)"""
        ...
        if file_path.suffix.lower() == '.pdf':
            ...
            if is_scanned:
                ...
                if not OCR_SUPPORT:
                    doc.close()
                    raise Exception("This PDF needs OCR. Install: pip install pytesseract pillow\n"
                                  "    Also install Tesseract: https://github.com/tesseract-ocr/tesseract")
            else:
                print(f"  📝 Text-based PDF - direct extraction")
    ...
    def ingest_book(self, file_path, book_info):
        ...
        # Read text (handles PDF or TXT with OCR if needed)
        try:
        ...
        if len(text) < 1000:
            print(f"  ⚠️  Warning: Only {len(text)} characters extracted. File may be corrupted or heavily scanned.")
...
def main():
    ...
    while True:
        ...
        if cmd == "quit":
            ...
            print("Unknown command. Use: ingest, query, stats, or quit")
